...
Total above-ground biomass (TAGB) estimates for one year old tropical forest succession are scarce in the literature. In their comprehensive review by Martin et al. (2013) compiled few regenerational plots one year after abandonment, from which TAGB can be estimated to be $\sim$ 14 Mg $\times$ ha $^{-1}$ (which gives `r 14 * 1000 * 0.0025` kg $\times$ 25 m$^{2}$ - vegetation plot used in our study). However, source studies were restricted only to secondary succession after pasture abandonment, which may affect biomass accumulation rates. In different study Mascaro et al. (2005) found that for forest gap succession following hurricane disturbance biomass after one year ranged widely with mean 78 $\pm$ 15 Mg $\times$ ha $^{-1}$ (`r 78 * 1000 * 0.0025` kg $\times$ 25 m$^{2}$). In our case we expected for the average biomass of experimental  plot to fall between these two values and we treated the above estimates as a lower and upper limit for our estimates. Firstly because we expect higher biomass accumulation rates in the primary forest matrix than in spontaneous succession in pastures. Secondly because we prepared plots in similar fashion trying to minimise unpredicted differences in gaps created after hurrican disturbance with variable initial conditions for regeneration. Study by Mascaro et al. (2005) provided us with estimates of variation, which from the same reason as  above we treated to be an upper limit for our system. Therefore we confidently assumed that in our 25 m$^2$ plots we would found approximately 100 kg of plant biomass with both $\sigma_{a}$ and $\sigma_{\varepsilon}$ of around 10 kg.
